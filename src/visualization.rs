...
use crate::analysis::{SimulationAnalysis, PriceHistory};
...
pub fn price_chart(price_history: &PriceHistory, width: usize, height: usize) -> String {
    ...
    let max_price = all_prices.iter()
        .map(|(_, p, _)| *p)
        .max()
        .unwrap_or(Decimal::ZERO);
    let min_price = all_prices.iter()
        .map(|(_, p, _)| *p)
        .min()
        .unwrap_or(Decimal::ZERO);
    let max_tick = all_prices.iter()
        .map(|(t, _, _)| *t)
        .max()
        .unwrap_or(0);
...
pub fn population_chart(analysis: &SimulationAnalysis, width: usize) -> String {
    ...
    let max_pop = analysis.villages.iter()
    ...
    for village in &analysis.villages {
        ...
        if change > 0 {
            let growth_width = (change as usize * (width - 20) / max_pop).min(width - 20 - initial_width);
            for _ in 0..growth_width {
                chart.push('█');
            }
        }
        
        chart.push_str(&format!(" {} → {} ({:+.0}%)\n", 
...
pub fn trade_flow_diagram(analysis: &SimulationAnalysis) -> String {
    ...
    for village in &analysis.villages {
        ...
        if village.trading_summary.executed_sells > 0 {
            diagram.push_str(&format!("  → SOLD: {} orders ({:.2} earned)\n",
                village.trading_summary.executed_sells,
                village.trading_summary.total_earned
            ));
        }
        
        if village.trading_summary.executed_buys > 0 {
            diagram.push_str(&format!("  ← BOUGHT: {} orders ({:.2} spent)\n",
                village.trading_summary.executed_buys,
                village.trading_summary.total_spent
            ));
        }
        
        let net = village.trading_summary.net_profit;
        if net != Decimal::ZERO {
            diagram.push_str(&format!("  {} NET: {:.2}\n",
    ...
    if !any_trades {
        ...
        diagram.push_str(&format!("Trade Success Rate: {:.1}%\n", analysis.market.trade_success_rate * 100.0));
    }
...
pub fn resource_timeline(events: &[crate::events::Event], village_id: &str, width: usize) -> String {
    ...
    timeline.push('\n');
    timeline.push_str(&format!("        0{:>width$}{}\n", max_tick, "", width = chart_width - 2));
    timeline.push_str("\nF=Food  W=Wood  █=Both\n");
...
pub fn strategy_matrix(analyses: &[SimulationAnalysis]) -> String {
    ...
    // Header
    matrix.push_str(&format!("{:>15} │ {:>8} │ {:>8} │ {:>8} │ {:>8}\n",
        "Strategy", "Growth%", "Survival%", "Trades", "Profit"
    ));
    matrix.push_str(&format!("{:─>15}─┼{:─>10}┼{:─>10}┼{:─>10}┼{:─>10}\n",
        "", "", "", "", ""
    ));
    ...
    for strategy in strategies {
        ...
        if !growth_rates.is_empty() {
            ...
            matrix.push_str(&format!("{:>15} │ {:>8.1} │ {:>8.1} │ {:>8} │ {:>8.2}\n",
                strategy,
                avg_growth * 100.0,
                avg_survival * 100.0,
                avg_trades,
