...
// === SURVIVAL STRATEGY ===
/// Prioritizes immediate survival needs
...
impl Strategy for SurvivalStrategy {
    ...
    fn decide_allocation_and_orders(
        ...
        let food_days = if food_per_day > dec!(0) {
            (village.food / food_per_day).to_u32().unwrap_or(0)
        } else {
            999
        };

        let wood_days = if wood_per_day > dec!(0) {
            (village.wood / wood_per_day).to_u32().unwrap_or(0)
        } else {
            999
        };
        ...
        if food_days < 10 && village.money > dec!(20) {
            ...
            let max_price = village.money / Decimal::from(quantity) * dec!(0.8); // Don't spend all money
            let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(1.1); // 10% above market
        ...
        if wood_days < 10 && village.money > dec!(20) {
            ...
            let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(1.1);
        ...
        if food_days > self.min_food_days * 2 {
            ...
            if quantity > 0 {
                let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(0.9);
        ...
        if wood_days > self.min_wood_days * 2 {
            ...
            if quantity > 0 {
                let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(0.9);
...
// === GROWTH STRATEGY ===
/// Focuses on population expansion
...
impl Strategy for GrowthStrategy {
    ...
    fn decide_allocation_and_orders(
        ...
        if need_houses && village.wood < dec!(30) && village.money > dec!(50) {
            let quantity = 20u32;
            let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(1.2);
            wood_bid = Some((price, quantity));
        }

        // Need food for population
        let food_per_day = Decimal::from(village.workers);
        let food_days = (village.food / food_per_day).to_u32().unwrap_or(0);
        if food_days < 30 && village.money > dec!(30) {
            let quantity = (30 * village.workers as u32).min(100);
            let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(1.15);
        ...
        if village.wood > dec!(100) && !need_houses {
            let quantity = 20u32;
            let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(0.85);
            wood_ask = Some((price, quantity));
        }
...
// === TRADING STRATEGY ===
/// Specializes in one resource and trades
    ...
    fn decide_allocation_and_orders(
        ...
        if self.specialize_food {
            ...
            if village.food > dec!(20) {
                ...
                let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(0.95);
                food_ask = Some((price, quantity));
            }
            ...
            if village.wood < dec!(20) && village.money > dec!(10) {
                let quantity = 30u32;
                let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(1.05);
                wood_bid = Some((price, quantity));
            }
        } else {
            ...
            if village.wood > dec!(10) {
                ...
                let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(0.95);
                wood_ask = Some((price, quantity));
            }
            ...
            if village.food < dec!(30) && village.money > dec!(10) {
                ...
                let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(1.05);
                food_bid = Some((price, quantity));
            }
        }
...
// === BALANCED STRATEGY ===
/// Adaptive strategy that responds to current needs
#[derive(Default)]
...
impl Strategy for BalancedStrategy {
    ...
    fn decide_allocation_and_orders(
        ...
        let food_urgency = 1.0 / (1.0 + food_days as f64 / 10.0);
        let wood_urgency = 1.0 / (1.0 + wood_days as f64 / 20.0);
...
// === GREEDY STRATEGY ===
/// Maximizes immediate production
#[derive(Default)]
...
impl Strategy for GreedyStrategy {
    ...
    fn decide_allocation_and_orders(
        ...
        if village.food < Decimal::from(village.workers) && village.money > dec!(10) {
            ...
            let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(1.5); // Will pay premium
            food_bid = Some((price, quantity));
        }

        if village.wood < dec!(1) && village.houses > 0 && village.money > dec!(10) {
            let quantity = 10u32;
            let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(1.5);
            wood_bid = Some((price, quantity));
        }
        ...
        if village.food > Decimal::from(village.workers * 2) {
            let quantity = (village.food - Decimal::from(village.workers))
                ...
                .min(100);
            if quantity > 0 {
                let price = market.last_food_price.unwrap_or(dec!(1.0)) * dec!(0.8); // Will sell cheap
                food_ask = Some((price, quantity));
            }
        }
        ...
        if village.wood > dec!(2) {
            ...
            if quantity > 0 {
                let price = market.last_wood_price.unwrap_or(dec!(5.0)) * dec!(0.8);
                wood_ask = Some((price, quantity));
            }
        }
